...
aligned(Board, Player, Size, Count, IsBlocked) :-
    length(Board, BoardLength),
    BoardDimension is round(sqrt(BoardLength)),
	alignedHori(Board, 0, Player, Size, HCount, 0, BoardLength, BoardDimension, IsBlocked),
	alignedVert(Board, 0, Player, Size, VCount, 0, BoardLength, BoardDimension, IsBlocked),
	alignedLeftDiag(Board, 0, Player, Size, LDCount, 0, BoardLength, BoardDimension, IsBlocked),
	alignedRightDiag(Board, 0, Player, Size, RDCount, 0, BoardLength, BoardDimension, IsBlocked),
	Count is HCount + VCount + LDCount + RDCount.

alignedHori(_, BoardLength, _, _, TotalCount, TotalCount, BoardLength, _, _).
alignedHori(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	...
	horiOfSize(Board, Acc, Player, 0, Size, BoardDimension, IsBlocked),
	NewAcc is Acc + 1,
	NewAccCount is AccCount + 1,
	alignedHori(Board, NewAcc, Player, Size, TotalCount, NewAccCount, BoardLength, BoardDimension, IsBlocked),
	!.
alignedHori(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	NewAcc is Acc + 1,
	alignedHori(Board, NewAcc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked),
	!.

alignedVert(_, BoardLength, _, _, TotalCount, TotalCount, BoardLength, _, _).
alignedVert(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	LastIndex is BoardLength-(Size-1)*BoardDimension-1, %% 76=11*11-4*11-1
    Acc =< LastIndex,
	not(isPosEmpty(Board, Acc)),
	nth0(Acc, Board, Player),
	vertOfSize(Board, Acc, Player, 0, Size, BoardDimension, IsBlocked),
	NewAcc is Acc + 1,
	NewAccCount is AccCount + 1,
	alignedVert(Board, NewAcc, Player, Size, TotalCount, NewAccCount, BoardLength, BoardDimension, IsBlocked),
	!.
alignedVert(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	NewAcc is Acc + 1,
	alignedVert(Board, NewAcc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked),
	!.

alignedLeftDiag(_, BoardLength, _, _, TotalCount, TotalCount, BoardLength, _, _).
alignedLeftDiag(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	LastIndex is BoardLength-(Size-1)*BoardDimension-1,
    Acc =< LastIndex,
	Acc mod BoardDimension >= (Size-1),
	not(isPosEmpty(Board, Acc)),
	nth0(Acc, Board, Player),
	leftDiagOfSize(Board, Acc, Player, 0, Size, BoardDimension, IsBlocked),
	NewAcc is Acc + 1,
	NewAccCount is AccCount + 1,
	alignedLeftDiag(Board, NewAcc, Player, Size, TotalCount, NewAccCount, BoardLength, BoardDimension, IsBlocked),
	!.
alignedLeftDiag(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	NewAcc is Acc + 1,
	alignedLeftDiag(Board, NewAcc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked),
	!.

alignedRightDiag(_, BoardLength, _, _, TotalCount, TotalCount, BoardLength, _,_).
alignedRightDiag(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension,IsBlocked) :-
	...
	not(isPosEmpty(Board, Acc)),
	nth0(Acc, Board, Player),
	rightDiagOfSize(Board, Acc, Player, 0, Size, BoardDimension, IsBlocked),
	NewAcc is Acc + 1,
	NewAccCount is AccCount + 1,
	alignedRightDiag(Board, NewAcc, Player, Size, TotalCount, NewAccCount, BoardLength, BoardDimension, IsBlocked),
	!.
alignedRightDiag(Board, Acc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked) :-
	NewAcc is Acc + 1,
	alignedRightDiag(Board, NewAcc, Player, Size, TotalCount, AccCount, BoardLength, BoardDimension, IsBlocked),
	!.
